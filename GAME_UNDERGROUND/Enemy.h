...
private:
	...
	Tool* tool = nullptr;
public:
	Enemy(int n) :power(CHAR[n-1][0]), name(ENEMY_NAMES[n-1]), health(CHAR[n-1][1]), cur_health(CHAR[n-1][1]), experience(CHAR[n-1][2]){}; //добавить настройку предмета
	Enemy(int n, std::string s) :power(CHAR[n-1][0]), name(s), health(CHAR[n-1][1]), cur_health(CHAR[n-1][1]), experience(CHAR[n-1][2]) {}; //добавить настройку предмета
